...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  const addTranscript = useCallback((update: TranscriptUpdate) => {
    ...
    setTranscripts((prev) => {
      ...
      // Upsert by sequence_id: partial/final updates for the same segment replace existing row.
      const existingIndex = prev.findIndex(
        (t) => t.sequence_id === newTranscript.sequence_id,
      );
      if (existingIndex >= 0) {
        const updated = [...prev];
      ...
      // Fallback dedupe for edge cases where sequence_id isn't stable.
      const existsByTextAndTime = prev.some(
        (t) => t.text === update.text && t.timestamp === update.timestamp,
      );
      if (existsByTextAndTime) {
        console.log(
