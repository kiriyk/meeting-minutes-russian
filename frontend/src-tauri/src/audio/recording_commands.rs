...
fn is_live_asr_mode_enabled() -> bool {
    let (enabled, _port, t_one_enabled, gigaam_enabled) = crate::get_asr_gateway_config_internal();
    enabled && (t_one_enabled || gigaam_enabled)
}
...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    if is_live_asr_mode_enabled() {
        info!("📡 Live ASR mode enabled: skipping built-in model validation");
    } else {
